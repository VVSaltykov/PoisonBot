            ...
            TelegramBotClient client)
        {
            ...
            try
            {
                ...
                else if (message.Type == MessageType.Contact)
                {
                    ...
                    if (promocodeTrue || e.Message.Text == "нет" || e.Message.Text == "Нет" || e.Message.Text == "НЕТ" )
                    {
                        ...
                        if (user.Role == Definitions.Role.User) await client.SendTextMessageAsync(message.Chat.Id, "Бот \"VaultKix\" - это инновационное решение, разработанное специально для удобного оформления заказов и расчета их стоимости на платформе Poizon. Он предоставляет возможность подобрать размер обуви и одежды, используя удобные таблицы и рекомендации. 😊\r\n\r\nНо функционал бота не ограничивается только этим. Он также предоставляет актуальную информацию о курсе доставки на текущий день, чтобы вы могли точно рассчитать стоимость доставки вашего заказа. 🚚💰\r\n\r\n\"VaultKix\" обеспечивает прямую связь с менеджером, который всегда готов помочь вам ответить на все вопросы, связанные с доставкой и выкупом товаров. Вы можете задать любые вопросы и получить оперативные ответы и рекомендации от опытного специалиста. 💬👩‍💼\r\n\r\nКроме того, бот также поможет вам подобрать обувь и одежду из наличия, чтобы ускорить процесс покупки. Вы сможете узнать, какие товары доступны прямо сейчас, чтобы сэкономить время на ожидании. ⌛️💡", replyMarkup: Buttons.StartMenu());
                        showButton = false;
                    }
                    else
                    {
